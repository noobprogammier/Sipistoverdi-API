from utls.sockets import create_socks, errors; from hashlib import sha256; from datetime import datetime, date; from json import loads, dumps
from utls.xmlCreator import create_XMLObject
from utls.encipher import Encipher
...
class requestAPI(create_socks):
	...
	def postReqXML(self):
		def createObjects(value:str, mem:list) -> str:
			...
			return text.clear()
		memory = []
		options_known = {"myInfo":"myInformation"} # The second key is the directory in the URL.
		...
		ksock = create_socks(cabundle="", defcont=False, sslx=False, port=0).getItem
...
xz_ = requestAPI(default_contextx=False,ssl=False, port=56152)
xz_.set_verbose = True
xz_.set_option = "myInfo"
output = xz_.postReqXML
